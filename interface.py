...
def view_merged_flight():
    ...
    # ==============================================================
    # 5. EL ARREGLO ESTÁ AQUÍ: DEFINIMOS TERMINAL Y PUERTA ANTES DEL TEXTO
    # ==============================================================
    assigned_gate = getattr(target_flight, 'gate', '---')
    terminal_dest = "T1"  # ESTO EVITA TU ERROR, SIEMPRE HABRÁ UN VALOR
    ...
    # ¡AQUÍ ES DONDE EXPLOTABA! Como ahora terminal_dest está definida arriba, ya no dará error
    ...
    tk.Label(info_frame, text=title_text, bg="#F0F4F8", font=("Helvetica", 10, "bold"), fg="#1A1A1A").pack(anchor="w",
                                                                                                           padx=10,
                                                                                                           pady=(10, 2))
    tk.Label(info_frame, text=f"STAR: {star} | SID: {sid}", bg="#F0F4F8", font=("Helvetica", 9), fg="#34495E").pack(
    ...
    def draw_and_animate():
        ...
        def phase3_departure():
            ...
            if sid == "DCT" or sid == "":
                ...
                # Escribir el nombre de todos los waypoints
        ...
        # ==========================================
        # FASE 2: MOVIMIENTOS EN TIERRA (TAXI)
        # ==========================================
        def phase2_ground():
            canvas.delete("all")

            # Dibujar mapa de tierra
            color_rwy = "#5D6D7E";
            color_line = "#FFFFFF"
            r1_y = h * 0.35;
            r1_start, r1_end = w * 0.90, w * 0.30
            r2_y = h * 0.65;
            r2_start, r2_end = w * 0.80, w * 0.20
            twy_n_y = h * 0.22;
            twy_c_y = h * 0.45;
            twy_s_y = h * 0.55
            cx_t1, cy_t1 = w * 0.15, h * 0.50
            ...
            # Pistas
            canvas.create_line(w * 0.35, h * 0.85, w * 0.55, h * 0.15, fill=color_rwy, width=35, capstyle=tk.ROUND)
            canvas.create_line(r1_start, r1_y, r1_end, r1_y, fill=color_rwy, width=35, capstyle=tk.ROUND)
            canvas.create_line(r2_start, r2_y, r2_end, r2_y, fill=color_rwy, width=35, capstyle=tk.ROUND)
            canvas.create_line(r1_start, r1_y, r1_end, r1_y, fill=color_line, width=2, dash=(12, 12))
            canvas.create_line(r2_start, r2_y, r2_end, r2_y, fill=color_line, width=2, dash=(12, 12))
...
btn_row(left, [("Load Structure",  load_airport_structure),
               ("Assign Gates",    assign_dynamic_gates)])
btn_row(left, [("Gate Diagram", show_info_gate_diagram),
...
btn_row(left, [("Day Occupancy",   plot_day_occupancy),
               ("Interactive Map", show_interactive_diagram)])
...
# --- SECCIÓN: EXTRA FUNCTIONS ---
extra_frame = tk.LabelFrame(left, text=" Extra Functions ", bg="#F0F4F8",
                            font=("Helvetica", 11, "bold"), fg="#3A7FC1", pady=10, padx=10)
# El pady de 20 por arriba es para darle un poco de aire y separarlo de los botones anteriores
extra_frame.pack(fill="x", padx=10, pady=(20, 10))

# Botón Active Runways
btn_runways = tk.Button(extra_frame, text="Active Runways", command=check_active_runways,
                        bg="#34495E", fg="white", relief="flat", pady=5)
btn_runways.pack(fill="x")

# Botón SIDs
btn_row = tk.Button(extra_frame, text="Assign SIDs", command=assign_sids_to_departures,
                    bg="#34495E", fg="white", relief="flat", pady=5)
btn_row.pack(fill="x", pady=(5, 0))

# Botón para Ver la salida de un avión por matrícula
btn_view_dep = tk.Button(extra_frame, text="View Aircraft Departure", command=view_aircraft_departure,
                         bg="#34495E", fg="white", relief="flat", pady=5)
btn_view_dep.pack(fill="x", pady=(5, 0))

# Botón para Asignar las STARs a las llegadas
btn_assign_stars = tk.Button(extra_frame, text="Assign STARs", command=assign_stars_to_arrivals,
                             bg="#34495E", fg="white", relief="flat", pady=5)
btn_assign_stars.pack(fill="x", pady=(5, 0))

# Botón para Ver la llegada de un avión
btn_view_arr = tk.Button(extra_frame, text="View Aircraft Arrival", command=view_aircraft_arrival,
                         bg="#34495E", fg="white", relief="flat", pady=5)
btn_view_arr.pack(fill="x", pady=(5, 0))

# Botón para ver el esquema del aeropuerto
btn_airport_map = tk.Button(extra_frame, text="View Airport Map", command=view_airport_map,
                            bg="#27AE60", fg="white", relief="flat", pady=5)
btn_airport_map.pack(fill="x", pady=(5, 0))

# Botón para ver los movimientos en tierra (Rodaje)
btn_ground = tk.Button(extra_frame, text="View Ground Taxiing", command=view_ground_movements,
                       bg="#D35400", fg="white", relief="flat", pady=5)
btn_ground.pack(fill="x", pady=(5, 0))

# Botón para Ver el ciclo completo de un avión fusionado
btn_view_merged = tk.Button(extra_frame, text="View Merged Flight", command=view_merged_flight,
                            bg="#8E44AD", fg="white", relief="flat", pady=5)
btn_view_merged.pack(fill="x", pady=(5, 0))
